...
try:
    ...
    raise RuntimeError(
        f'Usage monitor package not found. Tried {INSTALLED_DASHBOARD} and {REPO_ROOT}. Run ./install.sh or execute from repo.'
    ) from exc
        ...
        for section, paths in sorted(result.items()):
            if section == "warnings":
                ...
                for p in paths:
                    print(f"       → {p}")
        print()
        print("Done! Usage Monitor installed.")
        print()
        skill_scripts = HERMES_HOME / "skills" / "productivity" / "api-usage-monitoring" / "scripts"
        print(f"  {skill_scripts / 'usagectl.py'} status")
        return 0
...
def install_skill(repo: Path | None = None, *, skip_plugin: bool = False) -> dict[str, list[str]]:
    ...
    wrapper_path = local_bin / "usagemon"
    wrapper_path.write_text("""#!/usr/bin/env bash
# usagemon — API Usage Monitor CLI (status, serve, menubar, autostart, latest, install)
PY="${HOME}/.hermes/hermes-agent/venv/bin/python"
[ -x "${PY}" ] || PY="$(command -v python3)"
exec "${PY}" "${HOME}/.hermes/skills/productivity/api-usage-monitoring/scripts/usagectl.py" "$@"
""")
    wrapper_path.chmod(0o755)
    installed.setdefault("cli_wrapper", []).append(str(wrapper_path))
